...
//     it("Successfully deploy and fund smart wallet contract with 5 algos", async () => {
//         assert.isDefined(appId);
//         assert.equal(appInfo.amount, 5e6);
//     });
...
//         assert.equal(appGlobalStateDecodedObject.threshold, "2"); //threshold
...
//             if (new TextDecoder().decode(box.name) === boxName) {
//                 containsBox = true;
//             }
//         });
//         assert.isTrue(containsBox);
//     });
...
//         const optedIn = await isOptedIn(appAddr, assetId);
//         assert.isTrue(optedIn);
//     });

//     it("Smart Contract Wallet can successfully send algos to others", async () => {
//         // Create send algos transaction
//         const signer = algosdk.makeBasicAccountTransactionSigner(creator);
//         const receiver = algosdk.generateAccount();
//         const { result, boxName } = await sendAlgosTxn(
//             appId,
...
//         atc.addTransaction({ txn: txnDecoded, signer });
//         const txnResult = await atc.execute(algodClient, 4);
//         assert.isDefined(txnResult.txIDs[0]);
//     });

//     it("Owner can successfully remove box txn", async () => {
//         const signer = algosdk.makeBasicAccountTransactionSigner(creator);
//         // Add txn to box storage
//         const { result, boxName } = await sendOptInTxn(
//             appId,
//             creator.addr,
//             signer,
...
//             methodArgs: [boxName],
//             boxes: [
//                 { appIndex: 0, name: new Uint8Array(Buffer.from(boxName)) },
//             ],
//             ...commonParams,
//         };
//         const atc2 = new algosdk.AtomicTransactionComposer();
//         atc2.addMethodCall(removeTxnAppCall);
//         const optInTxn = await createOptInTxn(creator.addr, parseInt(appId));
//         if (optInTxn) {
//             console.log("Adding Opt-in txn.");
//             atc2.addTransaction({ txn: optInTxn, signer });
//         }
//         await atc2.execute(algodClient, 4);
//         const boxes = await algodClient
//             .getApplicationBoxes(parseInt(appId))
//             .do();
//         let containsBox = false;
