    ...
    switch (network) {
        case "TestNet":
            ...
            if (process.env.NEXT_PUBLIC_ALGOD_TOKEN_TESTNET !== undefined) {
                try {
                    algod_token = JSON.parse(
                        process.env.NEXT_PUBLIC_ALGOD_TOKEN_TESTNET
                    );
                } catch (e) {
            ...
            if (process.env.NEXT_PUBLIC_INDEXER_TOKEN_TESTNET !== undefined) {
                try {
                    indexer_token = JSON.parse(
                        process.env.NEXT_PUBLIC_INDEXER_TOKEN_TESTNET
                    );
                } catch (e) {
                    indexer_token =
                        process.env.NEXT_PUBLIC_INDEXER_TOKEN_TESTNET;
...
const getAlgodClient = (network) => {
    ...
    if (
        algod.token === undefined ||
        algod.address === undefined ||
        algod.port === undefined
    ) {
...
const getKmdClient = (network) => {
    ...
    const kmdClient = new algosdk.Kmd(kmd.token, kmd.address, kmd.port);
...
const getIndexerClient = (network) => {
    ...
    if (indexer.token === undefined)
        throw new Error("Unable to init Indexer Client");
